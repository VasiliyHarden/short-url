...
func (repo *DB) Save(code, originalURL string) (retCode string, inserted bool, err error) {
	err = repo.db.QueryRow(insertOnConflictShortURL, code, originalURL).Scan(&retCode, &inserted)
	if err != nil {
		return "", false, err
	}
	return retCode, inserted, nil
}
